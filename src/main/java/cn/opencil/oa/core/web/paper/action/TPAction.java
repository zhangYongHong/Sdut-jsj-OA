...
public class TPAction extends BaseAction<TrainingPaper> {
	...
	private static String shoolYear;
	...
	public String list() {
		...
		try {
			...
			if (tpQuery.getShoolYear() != "" || tpQuery.getShoolYear() != null) {
				tpQuery.setShoolYear(this.getModel().getSchoolYear());
			} else
				tpQuery.setShoolYear(DateUtil.groupSchoolYear());
			trainingPapers = this.tpService
					.getPageResultForTP(tpQuery);
			shoolYear = tpQuery.getShoolYear();
...
//			this.addFieldError("tpError", "更新成功！");
	...
	public String exportExcel() throws Exception {
		...
		excelFileGenerator
				.setTitle(StringUtils.isNotBlank(tpQuery.getShoolYear()) ? tpQuery
						.getShoolYear() : "" + " 实训试题归档汇总表");

		excelFileGenerator.setTitle(tpQuery.getShoolYear() + " 实训试题归档汇总表");
