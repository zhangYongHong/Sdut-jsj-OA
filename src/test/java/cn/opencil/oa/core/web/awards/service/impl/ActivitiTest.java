package cn.opencil.oa.core.web.awards.service.impl;

import cn.opencil.oa.core.web.activiti.service.ActivitiService;
import org.activiti.engine.RuntimeService;
import org.activiti.engine.repository.ProcessDefinition;
import org.junit.Test;
import org.junit.runner.RunWith;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.test.context.ContextConfiguration;
import org.springframework.test.context.junit4.SpringJUnit4ClassRunner;

import java.util.List;

/**
 * Created by mnzero on 16-8-10.
 */
@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(value = "classpath:cn/opencil/oa/core/spring/applicationContext-activiti.xml")
public class ActivitiTest {
    @Autowired
    ActivitiService activitiService;
    @Autowired
    RuntimeService runtimeService;
    @Test
    public void deployTest() {
//        activitiService.deploy();
    }

    @Test
    public void startTest() {
//        activitiService.deploy();
        activitiService.deployed();
//        activitiService.start();
    }

    @Test
    public void getDefList() {
        List<ProcessDefinition> list = activitiService.getDefinitions();
        for (ProcessDefinition processDefinition : list) {
            System.out.println(processDefinition.getId() + "----" + processDefinition.getName());
        }
    }

}
